...
def grab_banner(target, port):
    ...
    This reveals version info (useful for vulnerability checking!)
    """
    try:
        sock = socket.socket()
        sock.settimeout(2)
...
def scan_port(target, port, grab_banners=False):
    ...
    (This is the worker function for threads)
    """
...
def quick_scan(target, grab_banners=False):
    ...
    print(f"\n{Colors.CYAN}🚀 Quick scan of {len(common)} most common ports...{Colors.RESET}\n")
